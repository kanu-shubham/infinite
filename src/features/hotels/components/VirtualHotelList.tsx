...
const CompactHotelCard = memo(function CompactHotelCard({ hotel }: CardProps): JSX.Element {
  return (
    <div className="compact-card">
      <img className="compact-card__image" src={hotel.image} alt={hotel.name} loading="lazy" />
...
function ContainerVirtualList({ hotels }: StrategyProps): JSX.Element {
  const { containerRef, virtualItems, totalHeight, startIndex, endIndex } = useVirtualList({
  ...
  return (
    <div className="virtual-list__meta">
      ...
      <p className="virtual-list__info">Visible rows: {startIndex}–{endIndex}</p>
    </div>
  );
}

function WindowVirtualList({ hotels }: StrategyProps): JSX.Element {
  const { listRef, virtualItems, totalHeight, startIndex, endIndex } = useWindowVirtualizer({
    itemCount: hotels.length,
    itemHeight: ITEM_HEIGHT,
  });
...
export type VirtualStrategy = 'container' | 'window';
...
export default function VirtualHotelList({
  ...
  retry,
  strategy,
}: VirtualHotelListProps): JSX.Element {
  if (isLoading) {
  ...
  if (hotels.length === 0) {
    return <div className="virtual-list__empty">No hotels match your filters.</div>;
  }
